...
function delCurrentUser(){
    if(confirm("Are you sure to want to Logout?")==true){
...
function eventStatus(date,noOfDays){
    ...
    date.setHours(0);
    date.setMinutes(0);
    // console.log(date,endDate,noOfDays);
    let today=new Date();
...
// <a class="btn" id="${wizard.eventId}" onclick="viewEvent(this.id)"><i class="fa-solid fa-list-ul"></i></a></td>
...
const deleteEvent=(currentEventId)=>{
    ...
    let i;
    let eventsLst=[];
    for(i=0;i<tempEventsLst.length;i++){
        if(tempEventsLst[i]!==currentEventId){
            eventsLst.push(tempEventsLst[i]);
        }
    }
    ...
    eventsLst=[];
    for(i=0;i<tempEventLst.length;i++){
        if(tempEventLst[i].eventId!==currentEventId){
            eventsLst.push(tempEventLst[i]);
        }
    }
    tempEventLst=eventsLst;
