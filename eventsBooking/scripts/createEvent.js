...
if (tempcurrentEventId !== null) {
    ...
    for (i = 0; i < allEvents.length; i++) {
        if (allEvents[i].eventId === tempcurrentEventId) {
            currentEvent = allEvents[i];
            break;
        }
    }
    // localStorage.setItem("delete",tempcurrentEventId);
    ...
    // console.log(weekDaysObj,weekDays);
    for (let i = 0; i < weekDays.length; i++) {
        ...
        document.getElementById(startId).value = weekDaysObj[day].startTime;
        document.getElementById(endId).value = weekDaysObj[day].endTime;
    }
...
function slotEndTiming(dt1, dt2) {
    ...
    // console.log(d1Hrs,d1Minutes,d2Hrs,d2Minutes);
    if (d1Hrs > d2Hrs) {
        return false;
    }
    else if (d1Hrs === d2Hrs) {
...
const validateInputs = () => {
    ...
    // days count validate
    // if (tempcurrentEventId !== null) {
    //     if (daysValue < editNoOfDays) {
    //         result=false;
    //         setError(noOfDays, "No.Of Dates should be greater than previous One");
    //     }
    //     else {
    //         setSuccess(noOfDays);
    //     }
    ...
    for (let i = 0; i < weekDays.length; i++) {
        ...
        // console.log(startTime,endTime,slotEndTime);
